...
function RegisterMediator() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { name, phone, passwordHash: password };
    try {
      await axios.post(`${API_BASE}/api/register/mediator`, payload);
      setSubmitted(true);
  ...
  return (
    <Container maxWidth="xs">
      <Box sx={{ mt: 8 }}>
        <Typography variant="h5" align="center">Register as Mediator</Typography>
        {submitted ? (
          <Box sx={{ mt: 4 }}>
