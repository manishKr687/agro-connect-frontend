...
function RegisterFarmer() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { name, phone, passwordHash: password, agentId };
    try {
      await axios.post(`${API_BASE}/api/register/farmer`, payload);
      setSubmitted(true);
  ...
  return (
    <Container maxWidth="xs">
      <Box sx={{ mt: 8 }}>
        <Typography variant="h5" align="center">Register as Farmer</Typography>
        {submitted ? (
          <Box sx={{ mt: 4 }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <TextField label="Agent ID" value={agentId} onChange={e => setAgentId(e.target.value)} fullWidth margin="normal" autoComplete="off" />
